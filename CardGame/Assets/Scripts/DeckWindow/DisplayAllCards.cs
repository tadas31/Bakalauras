...
public class DisplayAllCards : MonoBehaviour
{
    ...
    public Goblin g = Goblin.instance;
    ...
    void Start()
    {
        ...
        List<AbstractCard> a = new List<AbstractCard>();
        for (int i = 0; i < 58; i++)
        {
            a.Add(g);
        }

        float rows = (float)Math.Ceiling(a.Count / 4f);
        float height = rows == 1 ? 24 * 2 + 455 : (24 * rows + 24) + (455 * rows);
        content.sizeDelta = new Vector2(0, height);
        ...
        foreach (var val in a)
        {
            ...
            Goblin.instance.getCard();
            //g.getCard();

            //GameObject card = val.getCard();


            GameObject newCard = Instantiate(Goblin.instance.getCard(), Vector3.zero, Quaternion.identity);
